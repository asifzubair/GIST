...
data
{
  ...
  real<lower=1> prior_lambda;

  int<lower=0> prior_index;
...
model
{
  ...
  real b_alpha = prior_lambda*prior_phi;
  real b_beta = prior_lambda*(1 - prior_phi);

  // alpha is a vector 1's, meaning this dirichlet represents a uniform prior.
  estimatedProportionsVecSimp ~ dirichlet(alpha);
  estimatedProportionsVecSimp[prior_index] ~ beta(b_alpha, b_beta);
