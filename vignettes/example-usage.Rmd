...
```{r}
sc_counts = read.table(file.path(test_data, "mouse-subsampled-scdata", "counts.tsv"), header = TRUE, row.names = 1)
...
```{r}
st_props = GIST(st_counts, sig_mat, num_cores = parallel::detectCores())
```
